...
while (1) {
   ...
   cat(if_else(n >= 10, "", " "), n, " ", today, " ", now, " : ", hsif_record, " ( - ", hsi_record, " = ", round(hsif_record - hsi_record), " )", " --- ",
       case_when(
   ...
   for (reference in hsif_references) {
      if ( (last_record <= reference) & (hsif_record > reference) ) { cat(" ", reference, sep = ""); beep(7)} # beep(1) beep(7) beep(10)
      if ( (last_record >= reference) & (hsif_record < reference) ) { cat(" ", reference, sep = ""); beep(11)} # beep(2) beep(9) beep(11)
   }
